...
class Autotrack(object):
    ...
    def scan(self,x,y):
        ...
        if x >= 0 and y >= 0 and x < self.__config.x and y < self.__config.y and self._map[x][y] != "b":
            return self._map[x][y]

    # 寻路到终点后，通过父坐标迭代返回并找到完整路径
    def trace(self,father):
        trace = [self._end] #终点的坐标一定是移动的最后一步，所以首先放入列表中
        # 循环F次
        ...
        for i in range(self._map[father[0]][father[1]][0][1]):
            ...
            trace.append(pos) #把刚刚找到的父节点记录进trace里
